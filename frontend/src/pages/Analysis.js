...
const Analysis = () => {
  ...
  // Helper function to determine risk severity
  const getRiskSeverity = (risk) => {
    ...
    if (highKeywords.some(keyword => riskLower.includes(keyword))) {
      return 'high';
    }
    
    // Medium severity keywords
    const mediumKeywords = ['may', 'could', 'should', 'recommend', 'suggest', 'consider', 'review', 'unclear', 'ambiguous', 'vague'];
    if (mediumKeywords.some(keyword => riskLower.includes(keyword))) {
      return 'medium';
    }
  ...
  // Get tooltip text for risk severity
  const getRiskTooltip = (severity) => {
    const tooltips = {
      high: 'High severity: Contains critical keywords like termination, liability, breach, or penalty',
      medium: 'Medium severity: Contains uncertain language like may, could, unclear, or ambiguous',
      low: 'Low severity: General risk without critical or uncertain keywords'
    };
  ...
  const getFilteredRisks = () => {
    if (!analysisResult?.analysis?.risks) return [];
    
    let filtered = analysisResult.analysis.risks;
  ...
  const getFilteredClauses = () => {
    if (!analysisResult?.analysis?.clauses) return [];
    
    let filtered = processClauses(analysisResult.analysis.clauses);
    ...
    if (searchQuery) {
      filtered = filtered.filter(clause => {
        ...
        return heading.toLowerCase().includes(query) || content.toLowerCase().includes(query);
      });
    }
        ...
        <>
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
            <div className="p-6">
              <div className="flex items-center gap-2 mb-6">
                ...
                <h2 className="text-lg font-semibold text-gray-900">Analysis Visualizations</h2>
              </div>
              
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {/* Risk Distribution Chart */}
                <div className="space-y-3">
                  <h3 className="text-sm font-medium text-gray-700">Risk Severity Distribution</h3>
                  <div className="bg-gray-50 p-4 rounded-lg">
                    <RiskDistributionChart risks={analysisResult.analysis.risks || []} />
                  </div>
                </div>

                {/* Clause Type Chart */}
                <div className="space-y-3">
                  <h3 className="text-sm font-medium text-gray-700">Clause Type Breakdown</h3>
                  <div className="bg-gray-50 p-4 rounded-lg">
                    <ClauseTypeChart clauses={analysisResult.analysis.clauses || []} />
                  </div>
                </div>
              </div>
            </div>
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                ...
                <div className="flex-1">
                  <div className="relative">
                    ...
                    <input
                      type="text"
                      placeholder="Search clauses and risks..."
                      value={searchQuery}
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
            <div className="p-6">
              ...
              {(() => {
                const finalClauses = getFilteredClauses();
                    ...
                    <div className="text-center py-8 bg-gray-50 rounded-lg">
                      ...
                      <p className="text-gray-500 text-base">
                        {searchQuery || filterSeverity !== 'all' 
                          ...
                          : 'No standard clauses identified in this document.'}
                      </p>
                    </div>
                  );
                }
                        ...
                        <summary className="cursor-pointer p-4 font-semibold text-blue-800 hover:bg-blue-50 transition-colors flex items-center justify-between">
                          ...
                          <div className="flex items-center gap-2">
                            <button
                              onClick={(e) => {
                                e.preventDefault();
                                handleCopyClause(clauseBlock.content, idx);
                              }}
